...
class Data
{
 static void Retrieve_MyCourtses() async {
   ...
   DatabaseReference DB_Refrance = await FirebaseDatabase.instance.reference()
       .child(
       "StudentCourse")
   ...
   }).then((value) async{await datacalling();
   ...
   //   //
   //   // }
   //   if(Student_CourceList.length){
   //     DatabaseReference DB_Reference1 =
   //     FirebaseDatabase.instance.reference().child("StudentCourse").child(
   //         auth.currentUser.uid).child(
   //         Student_CourceList.elementAt(0).toString());
   ...
   // }
    ...
    // myCoursesList.add(MyCoursesModel(
    //     RoomID:value.value['RoomID'], Courcename: value.value['Courcename'], cid: value.key,
    //     Teacher_Uid: value.value['Teacher_Uid'], SlotNo: value.value['SlotNo'],
    //     SlotTime: value.value['SlotTime'], Absents: value.value['Absents'], Day:value.value['Day'] ,
    //     StudentStrength:value.value['StudentStrength']
    // ));
  ...
  static void datacalling()async{
    ...
    for (int res = 0; res < Student_CourceList.length; res++) {
      ...
      // }
        ...
        DatabaseReference DB_Reference1 =
      await  FirebaseDatabase.instance.reference().child("StudentCourse").child(
            auth.currentUser.uid).child(
